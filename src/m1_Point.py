...
class Point(object):
    ...
    def move_to(self, x, y):
        ...
        self.x = x
        self.y = y
        self.distance_traveled += self.get_distance_from(Point(self.previous_x, self.previous_y))
        self.moves += 1
    ...
    def get_distance_from_start(self):
        return ((self.x - self.start_x) ** 2 + (self.y - self.start_y) ** 2) ** (1 / 2)

    def get_distance_traveled(self):
        return self.distance_traveled

    def closer_to(self, p2, p3):
        distance_to_p2 = self.get_distance_from(p2)
        distance_to_p3 = self.get_distance_from(p3)

        if distance_to_p2 <= distance_to_p3:
            return p2
        else:
            return p3
